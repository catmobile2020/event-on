...
{{--                            <div id="aria-notify-area"></div>--}}
                        ...
                        <div class="footer-video">
                            <ul>
                                @if ($auth_user->type == 1)
                                    <li>
                                        <button data-toggle="modal" data-target="#add-vote">
                                            Add Vote
                                        </button>
                                    </li>
                                    <li>
                                        <button data-toggle="modal" data-target="#question-att">
                                            Add Question
                                        </button>
                                    </li>
                                @else
                                    <li>
                                        <button data-toggle="modal" data-target="#voteModalResponse">
                                            Vote
                                        </button>
                                    </li>
                                    <li>
                                    ...
                                    <li>
                                        <button data-toggle="modal" data-target="#answerModalResponse">
                                            Answer
                                        </button>
                                    </li>
                                    <li>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form method="post" action="{{route('site.events.askSpeakerQuestion',$event->id)}}" class="submitFormData">
                            @csrf
                            ...
                            <div class="col-12">
                                <div class="block-content p-4">
                                    ...
                                    <div class="col-12 p-0 m-0">
                                        <div class="form-group">
                                            <textarea class="form-control" name="question" rows="3" placeholder="Ask"></textarea>
                                        </div>
                                    </div>
        ...
        <div class="modal fade" id="add-vote" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <ul class="nav nav-tabs" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link active" data-toggle="tab" href="#choose-vote-s" role="tab" aria-controls="choose-vote-s" aria-selected="false">Choose Vote</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link " data-toggle="tab" href="#add-vote-s" role="tab" aria-controls="add-vote-s" aria-selected="true">Add Vote</a>
                            </li>
                        </ul>
                        <div class="tab-content">
                            <div class="tab-pane fade" id="add-vote-s" role="tabpanel" aria-labelledby="add-vote-s">
                                <div class="block-content">
                                    <form method="post" action="{{route('site.polls.store',$auth_user->id)}}" class="submitFormData">
                                        @csrf
                                        <div class="alert alert-info sr-only insertResult"></div>
                                        ...
                                        <input type="submit" VALUE="Add Vote" class="btn btn-submit">
                                    </form>
                                </div>
                            </div>
        ...
        <div class="modal fade" id="question-att" tabindex="-1" role="dialog"  aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                        <div class="modal-header">
                    ...
                    <form method="post" action="{{route('site.questions.store',$auth_user->id)}}" class="submitFormData">
                        @csrf
                        <input type="hidden" name="event_id" value="{{$event->id}}"/>
                        <div class="modal-body">
                            <div class="alert alert-info sr-only insertResult"></div>
...
@section('js')
    ...
    <script>
        $(document).on('submit','.submitFormData',function (e) {
            e.preventDefault();
            let self = $(this);
            $.ajax({
                ...
                success:function (result) {
                    console.log(result);
                    $(self).find('.insertResult').html(result.message);
                    $(self).find('.insertResult').removeClass('sr-only');
                    if (result.status =='success')
                    {
                        self[0].reset();
                        setTimeout(function(){
                            $('#add-vote').modal('hide');
                            $('#question-att').modal('hide');
                            $('#ask-att').modal('hide');
                            $('#generalModalResponse').modal('hide');
                        }, 3000);
                    }
                },
                error:function (errors) {
                    $(self).find('.insertResult').html(Array.isArray(errors.responseJSON.message) ? errors.responseJSON.message.join('<br/>') : errors.responseJSON.message);
                    $(self).find('.insertResult').removeClass('sr-only');
                    setTimeout(function(){
                        $(self).find('.insertResult').addClass('sr-only');
                    }, 3000);
                }
            });
        });
...
@section('modals')
    <div class="modal fade" id="voteModalResponse" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" id="insertVoteModalData" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Not Found Voting Yet!</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
    ...
    <div class="modal fade" id="answerModalResponse" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" id="insertAnswerModalData" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Not Found Questions Yet!</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
